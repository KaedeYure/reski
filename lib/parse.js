...
export default function parse(string, initialData = {}, options = {}) {
  ...
  if (blocks.component.length > 0) {
    ...
    const maxPassCount = 5;
    let passCount = 0;
    let pendingTemplatesExist = true;
    
    while (pendingTemplatesExist && passCount < maxPassCount) {
      passCount++;
      pendingTemplatesExist = false;
      
      const checkForPendingTemplates = (comp) => {
    ...
    const cleanupInternalProps = (comp) => {
...
function processTemplateParams(component, templates, data, debug) {
  try {
    ...
    if (!component.raw || !Array.isArray(component.raw)) {
      if (debug) {
        console.error(`[Reski Error] Template parameters missing for component: ${templateName}`);
      }
      delete component._pendingTemplate;
      return;
    }
    ...
    for (let i = 0; i < component.raw.length && i < template.parameters.length; i++) {
      ...
      try {
        ...
        if (isValidType(processedValue, paramDefinition.type)) {
          ...
          console.error(`[Reski Error] Parameter type mismatch for '${paramDefinition.value}': expected ${paramDefinition.type}, got ${Array.isArray(processedValue) ? 'array' : typeof processedValue}`);
        }
      } catch (e) {
        if (debug) {
  ...
  if (component.children && Array.isArray(component.children)) {
    for (let i = 0; i < component.children.length; i++) {
      ...
      if (child.name && templates[child.name] && templates[child.name].definition && !child._processedTemplate) {
        ...
        processPendingTemplates(templateDef, templates, childContext, debug);
        
        if (child.classes) templateDef.classes = child.classes;
        if (child.props) templateDef.props = { ...(templateDef.props || {}), ...child.props };
        
        component.children[i] = templateDef;
      }
    }
  }
